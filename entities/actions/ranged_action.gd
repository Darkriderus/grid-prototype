...
func perform() -> bool:
	...
	if entity.distance(target.grid_position) > entity.stat_component.ranged_attack_range:
		if entity == get_map_data().player:
	...
	var attack_result := attacker.stat_component.ranged_try_to_hit(defender)
	...
	var dodge_result := defender.stat_component.try_to_dodge(attacker)
	...
	var damage := attacker.stat_component.ranged_damage(defender)
	var damage_given := defender.stat_component.take_damage(damage)
	if damage_given <= 0:
